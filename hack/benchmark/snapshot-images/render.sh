...
echo "rendering panels from $DASHBOARD"
python3 - "$SNAP_DIR" "$PORT_GRAFANA" "$UID_DASH" "$SLUG" "$FROM_MS" "$TO_MS" <<'PY'
...
curl -sf -o "$FULL" \
  "http://localhost:$PORT_GRAFANA/render/d/$UID_DASH/$SLUG?from=$FROM_MS&to=$TO_MS&width=1200&height=1400&tz=UTC" \
