...
var _ = Describe("gpuUsageViews", func() {
	...
	managedRequest := func(namespace, accelerator string, replicas int) pipeline.ModelScalingRequest {
	...
	It("passes the discovered figures through untouched as the physical view", func() {
		...
		views := gpuUsageViews([]pipeline.ModelScalingRequest{{Namespace: "team-a"}})
	...
	It("counts only WVA's own variants in the managed view", func() {
		...
		views := gpuUsageViews([]pipeline.ModelScalingRequest{managedRequest("team-a", "A100", 2)})
	...
	It("materialises a namespace being optimized that holds no GPUs, in both views", func() {
		...
		views := gpuUsageViews([]pipeline.ModelScalingRequest{
	...
	It("does not mutate the stored snapshot", func() {
		...
		_ = gpuUsageViews([]pipeline.ModelScalingRequest{{Namespace: "team-parked"}})
	...
	It("feeds a quota provider the managed view and a physical one the physical view", func() {
		...
		quota := pipeline.NewDefaultLimiter("q", pipeline.NewQuotaInventory(config.QuotaLimiterConfig{
		...
		physical := pipeline.NewDefaultLimiter("gpu", pipeline.NewTypeInventory("gpu", nil))
