...
// The shipped scaling-policy ConfigMaps must select the V2 (token/capacity-based)
...
var _ = Describe("shipped scaling-policy ConfigMap default entry", func() {
	DescribeTable("selects the V2 analyzer and validates",
		...
		Entry("kustomize base", "../../config/base/manager/scaling-policy-configmap.yaml"),
		Entry("deploy overlay", "../../deploy/configmap-scaling-policy.yaml"),
	)
})
