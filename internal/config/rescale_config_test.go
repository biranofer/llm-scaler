...
var _ = Describe("ScalingPolicy enableRescale", func() {
	...
	It("does not let a per-model override set enableRescale via Merge", func() {
		base := ScalingPolicy{EnableRescale: false}
		base.Merge(ScalingPolicy{EnableRescale: true, ModelID: "m", Namespace: "ns"})
...
var _ = Describe("rescale flag accessors", func() {
	...
	BeforeEach(func() {
		c = NewTestConfig()
		c.UpdateScalingPolicyConfig(map[string]ScalingPolicy{"default": {EnableRescale: true}})
	})
	...
	It("reads a namespace flag from that namespace's own config", func() {
		c.UpdateScalingPolicyConfigForNamespace("team", map[string]ScalingPolicy{"default": {EnableRescale: true}})
	...
	It("does not leak the cluster flag onto a namespace whose own flag is off", func() {
		c.UpdateScalingPolicyConfigForNamespace("plain", map[string]ScalingPolicy{"default": {KvCacheThreshold: 0.8}})
